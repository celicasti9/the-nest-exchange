(define-constant fee-basis-points u100) ;; 1.0%
...
(define-read-only (get-token-balance)
  (contract-call? .mazukamba get-balance (as-contract tx-sender))
)
...
(define-private (provide-liquidity-first (stx-amount uint) (token-amount uint) (provider principal))
    (begin
      ...
      (try! (contract-call? .mazukamba transfer token-amount tx-sender (as-contract tx-sender)))
      ...
      (as-contract (contract-call? .mazukamba-lp mint stx-amount provider))
    )
)
...
(define-private (provide-liquidity-additional (stx-amount uint))
  (let (
      ...
      (liquidity-token-supply (contract-call? .mazukamba-lp get-total-supply))
    ...
    (begin 
      ...
      (try! (contract-call? .mazukamba transfer tokens-to-transfer tx-sender contract-address))
      ;; mint LP tokens to tx-sender
      ;; inside as-contract the tx-sender is the exchange contract, so we use tx-sender passed into the function
      (as-contract (contract-call? .mazukamba-lp mint liquidity-to-mint provider))
    )
  )
)
  ...
  (begin 
    ...
    (let (
      ...
      (begin
        ...
        (as-contract (contract-call? .mazukamba transfer tokens-to-pay contract-address user-address))
      )
    )
  )
)
  ...
  (begin 
    ...
    (let (
      ...
      (begin
        ...
        (try! (contract-call? .mazukamba transfer token-amount user-address contract-address))
        ;; transfer tokens from contract to user
  ...
  (begin
    (asserts! (> liquidity-burned u0) err-zero-tokens)
      ...
      (begin 
        ...
        (try! (contract-call? .mazukamba-lp burn liquidity-burned))
        ...
        (as-contract (contract-call? .mazukamba transfer tokens-withdrawn contract-address burner))
      )
    )
  )
)