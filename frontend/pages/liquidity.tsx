...
export default function LiquidityPage() {
  ...
  const provideLiquidity = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // (contract-call? .the-nest-exchange provide-liquidity u1000 u2000)
  ...
  const removeLiquidity = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const burnPostCondition = makeStandardFungiblePostCondition(
      ...
      FungibleConditionCode.Equal,
      burnAmount,
      createAssetInfo(contractOwnerAddress, "mazukamba-lp", "mazukamba-lp"),
    )
    ...
    const options: ContractCallRegularOptions = {
      ...
      onFinish: ({ txId }) => {
        addTransactionToast(
          txId,
          `Burning liquidity (${burnAmount.toLocaleString()} MAZU-LP)...`,
        )
      }
    }
  ...
  const makeExchangeRatioSection = () => {
    ...
    return <p>1 STX = <b>{+exchangeRatio.toFixed(6)}</b> Mazukamba</p>
  }
